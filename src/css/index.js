const windowWidth = window.innerWidth;
const windowHeight = window.innerHeight;

export const deafultLightColor = color => color || '#ffffff';
export const fadedLightColor = color => color || 'rgba(255, 255, 255, 0.5)';

export const deafultDarkColor = color => color || '#4a4a4a';
export const fadedDarkColor = color => color || 'rgba(100, 100, 100, 0.5)';

export const primaryColor = color => color || '#e74c3c';
export const secondaryColor = color => color || '#f1c40f';

export const widthLeftPanel = '250px';
export const widthMiddlePanel = '100%';
export const widthTopPanel = '100%';

export const heightTopPanel = '70px';

export const elementHeight = 30;
export const iconSize = 'lg';

export const margin = 10;
export const padding = 10;
export const borderRadius = 5;
